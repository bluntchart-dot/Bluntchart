/**
 * premium-reading-tool.ts   —   FORK of lib/reading-tool.ts
 *
 * PRODUCTION CODE MUST NOT IMPORT FROM THIS FILE.
 *
 * This is the Anthropic Messages tool-use wrapper for the hidden Premium
 * Reading Engine (only reachable via /internal/premium). It was forked from
 * lib/reading-tool.ts so the premium prompt / schema / model config can
 * evolve without any risk of affecting the live website reading.
 *
 * Import from `./premium-prompt` (the fork), NEVER from `./claude-prompt`.
 *
 * When ready to merge the premium version into the live site, replace
 * lib/reading-tool.ts + lib/claude-prompt.ts with the contents of this
 * directory in one deliberate cutover.
 */
