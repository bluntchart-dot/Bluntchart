/**
 * premium-prompt.ts   —   FORK of lib/claude-prompt.ts
 *
 * PRODUCTION CODE MUST NOT IMPORT FROM THIS FILE.
 *
 * This is the prompt / schema / voice contract for the hidden Premium
 * Reading Engine (only reachable via /internal/premium). It was forked from
 * lib/claude-prompt.ts so the internal playground can iterate on prompts,
 * beat structures, sections, and validation without touching the live
 * paid reading. Edit this file freely — it cannot affect production.
 *
 * When ready to promote the premium version to the live site, replace
 * lib/claude-prompt.ts with this file's contents in one deliberate cutover.
 */
